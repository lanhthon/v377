...
<body>
    ...
    function initializePhieuChuanBiHangPage() {
        ...
        if (!donhangId || donhangId === 'undefined') {
            $('#phieu-container').html(
                '<p class="text-red-500 text-center font-bold text-lg p-10">Lỗi: ID đơn hàng không hợp lệ. Vui lòng quay lại trang chi tiết đơn hàng và thử lại.</p>'
            );
            return;
        }

        // **THAY ĐỔI**: Sửa đường dẫn API từ '../api/...' thành 'api/...'
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                $('#phieu-container').html(
                    '<p class="text-red-500 text-center">Lỗi kết nối server. Vui lòng kiểm tra đường dẫn API và console để biết thêm chi tiết.</p>'
                );
            }
        });
    }
